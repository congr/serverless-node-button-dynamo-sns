...
const dynamoDbHelper = require('./common/dynamodb-helper')
const snsHelper = require('./common/sns-helper')

const TABLE = process.env.DYNAMODB_TABLE_BUTTON
 ...
 */
exports.pubSNS = (event, context, callback) => {
  console.log('Received event:', event)

  // create/get topic
  snsHelper.createTopic('aws-iot-button-sns-topic', (err, topicArn) => {
    if (err) {
      return callback(err)
    }
    ...
    dynamoDbHelper.putItem(TABLE, data).then(() => SNS.publish(params, callback))
    .catch(error => callback(null, {statusCode: 400, body: JSON.stringify(error)}))
  })
}
